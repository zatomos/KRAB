...
extension KrabApiGroups on KrabApi {
  ...
  Future<SupabaseResponse<List<Group>>> getUserGroups() async {
    final res = await _rpc<List<Group>>("get_user_groups",
        ...
        parse: (r) => (r['groups'] as List)
            .map((g) => Group.fromJson(g, instanceId: instanceId))
            .toList());
    ...
    final groupsList = await Future.wait(res.data!.map((group) async {
      final iconUrl = await _pictures.getUrl(
        group.id,
        bucket: 'group-icons',
  ...
  Future<SupabaseResponse<Group>> getGroupDetails(String groupId) async {
    final res = await _rpc<Group>("get_group_details",
        ...
        parse: (r) => Group.fromJson(r['group'], instanceId: instanceId));
    if (!res.success || res.data == null) return res;
  ...
  Future<SupabaseResponse<Group>> editGroupIcon(
    ...
    try {
      ...
      final storage = _client.storage.from("group-icons");
      ...
      final iconUrl = await _pictures.refresh(groupId, bucket: 'group-icons');
      await evictAvatar(instanceId, groupId);

      // The icon is uploaded either way; only the re-read of the group failed.
      // Say so, rather than reporting success with no group attached.
  ...
  Future<String?> resolveGroupIconUrl(String groupId) => _pictures.getUrl(
        groupId,
        bucket: 'group-icons',
        ttl: const Duration(hours: 1),
      );
  ...
  Future<SupabaseResponse<void>> deleteGroupIcon(String groupId) async {
    try {
      await _client.storage.from("group-icons").remove([groupId]);
      await _pictures.refresh(groupId, bucket: 'group-icons');
      await evictAvatar(instanceId, groupId);
  ...
  /// Leave a group.
  Future<SupabaseResponse<void>> leaveGroup(String groupId) =>
      _rpc("leave_group",
