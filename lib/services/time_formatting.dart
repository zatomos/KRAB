...
String timeAgoLong(BuildContext context, DateTime date) {
  ...
  if (diff.inMinutes < 60) {
    return context.l10n.time_ago(context.l10n.minutes_ago(diff.inMinutes));
  }
  if (diff.inHours < 24) {
    return context.l10n.time_ago(context.l10n.hours_ago(diff.inHours));
  }
  if (diff.inDays < 7) {
    return context.l10n.time_ago(context.l10n.days_ago(diff.inDays));
  }
  if (diff.inDays < 30) {
    return context.l10n
        .time_ago(context.l10n.weeks_ago((diff.inDays / 7).floor()));
  }

  return context.l10n
      .time_ago(context.l10n.months_ago((diff.inDays / 30).floor()));
}
