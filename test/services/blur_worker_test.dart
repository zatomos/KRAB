...
void main() {
  ...
  test('blurs a photo down to backdrop size', () async {
    final blurred = await worker.blur(_photo());
  ...
  test('keeps one worker up across jobs instead of one per photo', () async {
    await worker.blur(_photo());
    expect(worker.isRunning, isTrue);

    // Several at once are handed to that same worker
    final all = await Future.wait([
      worker.blur(_photo(width: 200, height: 200)),
      worker.blur(_photo(width: 640, height: 480)),
      worker.blur(_photo()),
    ]);
  ...
  test('gives back nothing for bytes that are not a photo', () async {
  ...
  test('starts again after being stopped', () async {
    ...
    expect(await worker.blur(_photo()), isNotNull);
    expect(worker.isRunning, isTrue);
  });

  test('stopping mid-job answers null rather than hanging', () async {
    final inFlight = worker.blur(_photo(width: 2000, height: 2000));
    worker.stop();
