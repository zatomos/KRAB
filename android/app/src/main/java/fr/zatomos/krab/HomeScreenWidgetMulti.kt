    ...
    companion object {
        ...
        suspend fun updateAppWidget(context: Context, manager: AppWidgetManager, id: Int) {
            try {
                ...
                val sysLimit = systemBitmapLimit(context)
                val safeLimit = (sysLimit * 0.95).toInt()
                ...
                val mainBudget = imageBudget * 55 / 95
                val prevBudget = imageBudget * 20 / 95
                Log.d(TAG, "Widget $id: sysLimit=$sysLimit pfpSlots=$pfpSlots mainBudget=$mainBudget prevBudget=$prevBudget")
                ...
                loadScaledBitmap(mainUrl, mainBudget)?.let { bm ->
                    withContext(Dispatchers.Main) {
                        views.setImageViewBitmap(R.id.multi_main_image, bm)
